...
<div class="container">
    <div class="panel-body">
        <form action="{{ route('admin.licence_grade.store') }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
                        ...
                        <div class="col-md-5">
                           <select class="form-control" name="lic_name_id">
                               ...
                               @foreach($licence_names as $licence_name)
                                <option value="{{$licence_name->id}}">{{$licence_name->lic_name}}</option>
                               @endforeach
                           </select>

                            @if ($errors->has('lic_name_id'))
                                <span class="help-block">
                                    <strong>{{ $errors->first('lic_name_id') }}</strong>
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="form-group">
                        <label for="" class="form-label col-md-2">လိုင်စင်အဆင့်သတ်မှတ်ချက်*</label>
                        <div class="col-md-5">
                            <input type="text" class="form-control" name="grade" id="grade" placeholder="က" value="{{ old('grade') }}"  />
