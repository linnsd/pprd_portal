...
<div class="container">
    <div class="panel-body">
        <form action="{{ route('admin.sub_lic_gp.update',$sublicgps->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                           ...
                           <select class="form-control" name="lic_gp_id">
                               ...
                               @foreach($licencegps as $licencegp)
                                    <option value="{{ $licencegp->id }}"
                                        {{ old('lic_gp_id', $sublicgps->lic_gp_id) == $licencegp->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="form-group">
                        ...
                        <div class="col-md-5">
                            <input type="text" class="form-control" name="lic_sub_gp_name" id="lic_sub_gp_name" placeholder="ရေနံထွက်ပစ္စည်းလုပ်ငန်းလိုင်စင်" value="{{ old('lic_sub_gp_name',$sublicgps->lic_sub_gp_name) }}"  />

                            @if ($errors->has('lic_sub_gp_name'))
                                <span class="help-block">
                                    <strong>{{ $errors->first('lic_sub_gp_name') }}</strong>
                                </span>
                            @endif
                        </div>
                    </div>
